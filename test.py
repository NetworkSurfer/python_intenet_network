...
for l in range(len(res['topology']['links'])):
    interface_id_and_physical_number_map = []
    for n in range(len(res['topology']['links'][l]['nodes'])):
        interface_id_and_physical_number_map.append([])
    #print(a)
    for m in range(len(res['topology']['links'][l]['nodes'])):
        interface_id_and_physical_number_map[m].append(res['topology']['links'][l]['nodes'][m]['node_id'])
        interface_id_and_physical_number_map[m].append(res['topology']['links'][l]['nodes'][m]['label']['text'])
        #link_node_map[res['topology']['links'][l]['link_id']] 
    link_node_map[res['topology']['links'][l]['link_id']] = interface_id_and_physical_number_map
